...
export class UsersPageComponent implements OnInit {
  ...
  readonly displayedColumns = ['name', 'email', 'roles', 'status', 'lastLoginAt', 'actions'];
  ...
  loadUsers(): void {
    ...
    this.administrationService.listUsers(
      this.pageIndex() + 1,
      this.pageSize(),
      ...
      error: (error: unknown) => this.snackBar.open(
  ...
  performAction(user: User, action: 'activate' | 'deactivate' | 'unlock'): void {
    ...
    request$.pipe(
      ...
      next: () => {
        this.snackBar.open(`User ${this.actionPastTense(action)}.`, 'Dismiss', { duration: 3000 });
        this.loadUsers();
      },
      error: (error: unknown) => this.snackBar.open(
        getApiErrorMessage(error, `The user could not be ${this.actionPastTense(action)}.`),
        'Dismiss',
        { duration: 5000 }
      )
    });
  ...
  private actionPastTense(action: 'activate' | 'deactivate' | 'unlock'): string {
