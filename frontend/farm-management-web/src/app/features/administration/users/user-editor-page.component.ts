...
function passwordsMatch(control: AbstractControl): ValidationErrors | null {
  ...
  const confirmation = control.get("confirmPassword")?.value as
    string | undefined;
...
export class UserEditorPageComponent implements OnInit {
  ...
  readonly isGlobalAdministrator =
  ...
  ngOnInit(): void {
    ...
    const organizations$ =
      !this.isEditing && this.isGlobalAdministrator
        ? this.farmManagementService.listOrganizations()
        : of({ items: [] as readonly Organization[] });
    const user$ =
      this.userId === null
        ? of(null)
        : this.administrationService.getUser(this.userId);
    forkJoin({ roles: roles$, user: user$, organizations: organizations$ })
      .pipe(
  ...
  submit(): void {
    ...
    const request$: Observable<User> = this.isEditing
      ? this.administrationService
          ...
          .pipe(
            switchMap((user) =>
              this.shouldUpdateRoles(roleIds)
                ? this.administrationService.assignUserRoles(user.id, {
                    roleIds,
                  })
                : of(user),
            ),
      ...
      : this.administrationService.createUser({
          ...
          organizationId:
            this.isGlobalAdministrator && value.organizationId.trim().length > 0
    ...
    request$
      ...
      .subscribe({
        next: () => {
          this.snackBar.open(
            this.isEditing ? "User updated." : "User created.",
            "Dismiss",
            { duration: 3000 },
          );
  ...
  private shouldUpdateRoles(roleIds: readonly string[]): boolean {
    return (
      this.permissionService.has("Users.ManageRoles") &&
      JSON.stringify([...roleIds].sort()) !==
        JSON.stringify(this.initialRoleIds)
    );
  }
}
