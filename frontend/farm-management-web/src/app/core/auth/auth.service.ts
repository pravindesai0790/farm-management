...
export class AuthService {
  ...
  login(credentials: LoginRequest): Observable<LoginResponse> {
    return this.http
      .post<LoginResponse>(`${this.authEndpoint}/login`, credentials, {
  ...
  refreshAccessToken(): Observable<RefreshResponse> {
    if (this.refreshRequest$ === null) {
      this.refreshRequest$ = this.http
        .post<RefreshResponse>(`${this.authEndpoint}/refresh`, null, {
          withCredentials: true,
        })
        .pipe(
  ...
  loadCurrentUser(): Observable<CurrentUser> {
    return this.http
      .get<CurrentUser>(`${this.authEndpoint}/me`)
  ...
  ensureAuthenticated(): Observable<boolean> {
    ...
    const accessToken$: Observable<RefreshResponse | null> =
      this.store.accessToken() === null ? this.refreshAccessToken() : of(null);
  ...
  logout(): Observable<void> {
    ...
    if (this.logoutRequest$ === null) {
      this.logoutRequest$ = this.http
        .post<void>(`${this.authEndpoint}/logout`, null, {
          withCredentials: true,
        })
        .pipe(
          finalize(() => {
