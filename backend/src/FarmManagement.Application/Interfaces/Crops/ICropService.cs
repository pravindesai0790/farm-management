...
{
    Task<PagedResponse<CropResponse>> ListCropsAsync(CropActor actor, int page, int pageSize, string? search, bool? isActive, CancellationToken cancellationToken = default);
    ...
    Task<PagedResponse<CropVarietyResponse>> ListVarietiesAsync(CropActor actor, Guid cropId, int page, int pageSize, bool? isActive, CancellationToken cancellationToken = default);
